...
private fun tenGod(dayStem:String, otherStem:String):String{
    ...
    val ed=elemOfStem(dayStem); val eo=elemOfStem(otherStem)
...
fun getSajuCompat(
    ...
    // 대운수(선택 반올림)
    val years = (abs(Duration.between(birth, term).toHours()).toDouble() / 24.0) / 3.0
    val daeNum = when(daeRound.lowercase()){
        "floor" -> kotlin.math.floor(years).toInt().coerceAtLeast(1)
        "round" -> kotlin.math.round(years).toInt().coerceAtLeast(1)
        else    -> kotlin.math.ceil(years).toInt().coerceAtLeast(1)
    }
    val startYear = info.sy + daeNum - 1
